...
from par_tsne_utils import calc_betas_loop
from par_tsne_utils import get_squared_cross_diff_np
...
def _get_normed_sym_tf(X_, batch_size):
    ...
    norm_facs = tf.math.reduce_sum(X_, axis=0, keepdims=True)
    X_ = X_ / norm_facs
...
class Parametric_tSNE(object):
    ...
    def _init_model(self):
        ...
        self.model = tf.keras.Sequential(self._all_layers)
    ...
    def _pretrain_layers(self, pretrain_data, batch_size=64, epochs=10, verbose=0):
        ...
        for ind, end_layer in enumerate(self._all_layers):
            ...
            ae = tf.keras.Sequential(cur_layers)
    ...
    def restore_model(self, model_path, training_betas=None, num_perplexities=None):
        ...
        self.model = tf.keras.models.load_model(model_path, custom_objects=cust_objects)
