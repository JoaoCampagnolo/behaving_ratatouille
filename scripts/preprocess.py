...
def patient_files(root_dir):
    patients = {}
    for subj in [f for f in os.listdir(root_dir) if not os.path.isfile(os.path.join(root_dir, f))]: #gets all folder names
        patients[subj]={}
        subj_path = root_dir+'/'+subj
        for date in [f for f in os.listdir(subj_path) if not os.path.isfile(os.path.join(subj_path, f))]:
            if date != '.ipynb_checkpoints':
                patients[subj][date]={}
                subj_date = subj_path+'/'+date
                for recording in [f for f in os.listdir(subj_date) if os.path.isfile(os.path.join(subj_date, f))]:
                    if 'teste' not in recording:
                        recording_path = subj_date+'/'+recording
                        data = pd.read_csv(recording_path)
                        patients[subj][date]={'Recording': recording, 'length': data.size}
    return patients
...
def filter_batch(pts, filter_indices=None, config=None, freq=None):
    ...
    if filter_indices is None:
        filter_indices = np.arange(headband_setup.num_electrodes)
    if config is None:
        config = {
            'freq': 220,  # Hz
            'mincutoff': 0.1,  # FIXME
    ...
    f = [[OneEuroFilter(**config) for j in range(pts.shape[-1])] for i in range(headband_setup.num_electrodes)]
